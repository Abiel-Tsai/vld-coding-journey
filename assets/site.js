...
let savedTheme;
try{savedTheme=localStorage.getItem('vld-theme')}catch{/* System theme still works when storage is unavailable. */}
...
function updateThemeLabel(){const theme=currentTheme();if(themeLabel)themeLabel.textContent=theme;if(themeColor)themeColor.setAttribute('content',theme==='dark'?'#1b211c':'#f5f2e9')}
...
if(themeButton){updateThemeLabel();themeButton.addEventListener('click',()=>{const next=currentTheme()==='dark'?'light':'dark';root.dataset.theme=next;try{localStorage.setItem('vld-theme',next)}catch{}updateThemeLabel()})}
